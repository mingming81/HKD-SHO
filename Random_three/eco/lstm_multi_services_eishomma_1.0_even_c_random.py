...
totalEpochRewards=np.full([100,4],0)

countReward=np.full([100,4],0)
...
# %%  
    ...
    for step in range(0,steps):
        ...
        if sigma>0.1:
            ...
            Q_temp_ac_t,Q_temp_tt_t=torch.randn((1,X_AC.shape[1])),torch.randn((1,X_T.shape[1]))
            Q_air_win_t,Q_air_ap_t,Q_air_at_t, Q_air_et_t=torch.randn((1,X_WIN.shape[1])),torch.randn((1,X_AP.shape[1])),torch.randn((1,X_T.shape[1])),torch.randn((1,X_ET.shape[1]))
        ...
        print(f'num_corr_all: {num_corr_all}')
        ...
        totalEpochRewards[epoch,3]+=all_reward
