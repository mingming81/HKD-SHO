...
countReward=np.full([num_epochs,4],0)
...
# %%   
    ...
    for step in range(0,steps):
        ...
        if sigma>0.1:
            ...
            Q_light_cur_t,Q_light_ls_t=torch.randn((1,X_CUR.shape[1])),torch.randn((1,X_LS.shape[1]))
            Q_temp_cur_t,Q_temp_ac_t,Q_temp_win_t,Q_temp_tt_t, Q_temp_et_t=torch.randn((1,X_CUR.shape[1])),torch.randn((1,X_AC.shape[1])),torch.randn((1,X_WIN.shape[1])),torch.randn((1,X_T.shape[1])),torch.randn((1,X_ET.shape[1]))
            Q_air_cur_t,Q_air_win_t,Q_air_ap_t,Q_air_at_t, Q_air_et_t=torch.randn((1,X_CUR.shape[1])), torch.randn((1,X_WIN.shape[1])),torch.randn((1,X_AP.shape[1])),torch.randn((1,X_T.shape[1])),torch.randn((1,X_ET.shape[1]))
        ...
        # %%
    countReward[epoch,0]=num_corr_light
    countReward[epoch,1]=num_corr_temp
    countReward[epoch,2]=num_corr_air
    countReward[epoch,3]=num_corr_all      
